...
Eigen::Quaternion<DataType> averageQuaternions(ForwardIterator const& begin,
...
template <typename DataType, typename ForwardIterator>
Eigen::Matrix<DataType, 3, 1> averagePositions(ForwardIterator const& begin,
                                               ForwardIterator const& end)
{
  if (begin == end)
  {
...
template <typename DataType, typename ForwardIterator>
Eigen::Transform<DataType, 3, Eigen::Isometry> averageIsometries(ForwardIterator const& begin,
                                                                 ForwardIterator const& end)
{
  ...
  return Eigen::Translation<DataType, 3>(averagePositions<DataType>(positions)) *
         averageQuaternions<DataType>(quaternions);
}
